---
title: "InsertRib Method (IFeatureManager)"
...
- `Is2Sided`: True if the rib is double-sided, false if the rib is single sided; a double-sided rib thickens on both sides of the sketch
- `ReverseThicknessDir`: True to thicken the opposite side of the sketch normal, false to not; only valid for single-sided ribs
- `Thickness`: Rib thickness
- `ReferenceEdgeIndex`: Edge in the sketch to use to determine the material direction and for draft reference; when the rib is drafted, the mid point of this edge maintains the thickness value; other points on the rib may have a different thicknesses due to the draft
...
[FeatureManager::InsertRib](ms-its:sldworksapivb6.chm::/sldworks~FeatureManager~InsertRib.html)
