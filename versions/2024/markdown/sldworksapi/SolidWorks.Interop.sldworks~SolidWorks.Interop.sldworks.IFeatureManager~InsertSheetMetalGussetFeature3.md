---
title: "InsertSheetMetalGussetFeature3 Method (IFeatureManager)"
project: "SOLIDWORKS API Help"
interface: "IFeatureManager"
member: "InsertSheetMetalGussetFeature3"
kind: "method"
source: "sldworksapi/SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IFeatureManager~InsertSheetMetalGussetFeature3.html"
---

# InsertSheetMetalGussetFeature3 Method (IFeatureManager)
...
- `BOffset`: True to offset the gusset section plane from the selected reference point, false to not
- `DOffset`: Value by which to offset the gusset section plane from the selected reference point; valid only if BOffset is true
...
- `ProfDimType`: Type of gusset profile dimensioning scheme:

- 0 = indent
- 1 = profile dimensioning

(see**Remarks**)
- `DIndentDepth`: Gusset depth; valid only if ProfDimType = 0
...
- `BFlipSides`: True to swap d1 and d2 in the dimensioning scheme, false to not; valid only if ProfDimType = 1 (see
...
- `GussetType`: Type of gusset:

- 0 = rounded back
- 1 = flat back
- `BEdgeFillet`: True to fillet the edges of a gusset, false to not; valid only if GussetType = 1
- `DEdgeFilletRadius`: Edge fillet radius; valid only if BEdgeFillet is true and GussetType = 1
- `BOverrideDoc`: True to override the document flat pattern properties, false to not
- `BShowProfile`: True to display the gusset profile when flattened, false to not; valid only if BOverrideDoc is true
- `BShowCenter`: True to display the center marks when flattened, false to not; valid only if BOverrideDoc is true
...
[FeatureManager::InsertSheetMetalGussetFeature3](ms-its:sldworksapivb6.chm::/sldworks~FeatureManager~InsertSheetMetalGussetFeature3.html)
