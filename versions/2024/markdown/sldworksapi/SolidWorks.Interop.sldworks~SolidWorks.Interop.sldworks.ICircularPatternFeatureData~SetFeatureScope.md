---
title: "SetFeatureScope Method (ICircularPatternFeatureData)"
...
- `FeatureScopeOption`: True (default) to specify affected bodies, false to apply the pattern to all bodies every time the feature regenerates (see

Remarks

)
- `AutoSelectBodies`: True (default) to automatically select all bodies intersected by this pattern feature, false to specify affected bodies (see
...
[CircularPatternFeatureData::SetFeatureScope](ms-its:sldworksapivb6.chm::/sldworks~CircularPatternFeatureData~SetFeatureScope.html)
