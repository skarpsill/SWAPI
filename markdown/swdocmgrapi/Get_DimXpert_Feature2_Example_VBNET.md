---
title: "Get More DimXpert Feature Examples (VB.NET)"
project: ""
interface: ""
member: ""
kind: "example"
source: "swdocmgrapi/Get_DimXpert_Feature2_Example_VBNET.htm"
---

# Get More DimXpert Feature Examples (VB.NET)

This example builds a part to demonstrate usage of the
SOLIDWORKS Document Manager API for the following DimXpert features:

kadov_tag{{<spaces>}}kadov_tag{{</spaces>}}*kadov_tag{{<spaces>}}kadov_tag{{</spaces>}}Compound
hole

kadov_tag{{<spaces>}}kadov_tag{{</spaces>}}*kadov_tag{{<spaces>}}kadov_tag{{</spaces>}}Compound
notch

'----------------------------------------------------------------------------

'kadov_tag{{<spaces>}}kadov_tag{{</spaces>}}1.
Open`public_documents`**\samples\tutorial\api\cover_with_dimensions.sldprt.**

'kadov_tag{{<spaces>}}kadov_tag{{</spaces>}}2.
Open the DimXpert toolbar from**View > Toolbars**

' (select the first instance
of Toolbars on the View menu).

'kadov_tag{{<spaces>}}3.
Observe the following DimXpert features on the

' DimXpertManager tab:kadov_tag{{<spaces>}}kadov_tag{{</spaces>}}**Simple
Hole2**and**Notch1**.

'kadov_tag{{<spaces>}}kadov_tag{{</spaces>}}4.
Close the part, saving it to another name.

'kadov_tag{{<spaces>}}NOTE:
Because this part is used in a SOLIDWORKS

' online tutorial, do not save
any changes to the original file name.

'kadov_tag{{<spaces>}}kadov_tag{{</spaces>}}5.
Read how to load and run[Code_Example_VBNET](Code_Example_VBNET.htm)with this part.

'---------------------------------------------------------------------------
