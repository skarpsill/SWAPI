---
title: "ModelDoc2::FeatureBossThin2"
project: ""
interface: ""
member: ""
kind: "topic"
source: "obsoleteapi/ModelDoc2/ModelDoc2__FeatureBossThin2.htm"
---

# ModelDoc2::FeatureBossThin2
...
This method creates a thin feature boss.

Syntax (OLE Automation)

void = ModelDoc2.FeatureBossThin2 ( sd, flip, dir,
...
| Input: | (VARIANT_BOOL) dir | TRUE if you want Direction 1 to be opposite the default direction |
...
from the face or plane toward the sketch |
| Input: | (VARIANT_BOOL) translateSurface1 | If you choose swEndcondOffsetFromSurface as the termination type for
...
| Input: | (VARIANT_BOOL) translateSurface2 | If you choose swEndcondOffsetFromSurface as the termination type for
...
| Input: | (long) revThinDir | Thin feature type: 0
= one direction 1
...
| Input: | (VARIANT_BOOL) addBends | TRUE to add auto-bends (open profile base features only) |
| Input: | (double) bendRad | Bend radii if addBends is TRUE |

Syntax (COM)

status = ModelDoc2->FeatureBossThin2 ( sd, flip,
...
| --- | --- | --- |
| Input: | (VARIANT_BOOL) flip | Not used |
| Input: | (VARIANT_BOOL) dir | TRUE if you want Direction 1 to be opposite the default direction, FALSE
if not |
| Input: | (long) t1 | Termination type for first end as defined in swEndConditions_e |
| Input: | (long) t2 | Termination type for second end as defined in swEndConditions_e |
| Input: | (double) d1 | Depth of extrusion for first end in meters |
...
from the face or plane toward the sketch |
| Input: | (VARIANT_BOOL) offsetReverse2 | TRUE specifies offset away from the sketch if you chose to offset the
first second condition from another face or plane, FALSE specifies offset
from the face or plane toward the sketch |
| Input: | (VARIANT_BOOL) translateSurface1 | If you choose swEndcondOffsetFromSurface as the termination type for
the first end, then TRUE specifies that the end of the extrusion is a
translation of the reference surface, FALSE specifies to use a true offset |
| Input: | (VARIANT_BOOL) translateSurface2 | If you choose swEndcondOffsetFromSurface as the termination type for
the second end,
then TRUE specifies that the end of the extrusion is a translation of
the reference surface, FALSE specifies to use a true offset |
| Input: | (double) thk1 | Wall thickness 1 (midplane type uses (thk1)/2 for each direction) |
| Input: | (double) thk2 | Wall thickness 2 (only used when thinType = 3) |
| Input: | (double) endThk | End cap thickness (only used when capEnds = 1) |
| Input: | (long) revThinDir | Thin feature type: Thin feature type: 0
= one direction 1
= one direction reverse 2
= midplane 3
= two direction |
...
<param name="_CURRENTFILEPATH" value="C:\cheryle\APIHelp\sw2007\obsoleteapi\ModelDoc2\ModelDoc2__FeatureBossThin2.htm" >
